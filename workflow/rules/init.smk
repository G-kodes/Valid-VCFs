...
def DataPrepRequirements() -> DataPrepList:
    ...
    for name, inputRegex in regexes.items():
        data[name] = [re.search(inputRegex, filename).group(1) for filename in glob.glob(
            os.path.join(dataDir, '*')) if re.search(inputRegex, filename)]

    results['gzVCF'] = {filename for filename in data['VCF']
                        if filename not in data['gzVCF']}
    results['TABIX'] = {filename for filename in data['gzVCF']
                        if filename not in data['TABIX']}
